    ...
    public class Certificador : ICertificador
    {
        async Task<FirmadoResponse> ICertificador.FirmarXml(FirmadoRequest request)
        {
            var task    =   Task.Factory.StartNew(()    =>
            {
                ...
                certificate.Import(request.CertificadoDigital,   request.PasswordCertificado,    X509KeyStorageFlags.MachineKeySet);
        ...
        FirmadoResponse ICertificador.FirmarXml(FirmadoRequest request, bool sincrono)
        {
            var response    =   new FirmadoResponse();
